...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-8 bg-zinc-950 text-slate-50 relative overflow-hidden">
      {/* Background Ambience */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(50,50,50,0.1)_0,rgba(0,0,0,0.4)_100%)] pointer-events-none" />
      ...
      <AnimatePresence mode="wait">
        {uiState === "IDLE" && (
          ...
          >
            ...
            <Button 
              size="lg" 
              className="w-full h-32 text-2xl font-bold rounded-3xl shadow-[0_0_40px_rgba(255,255,255,0.1)] active:scale-95 transition-all"
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <Skeleton className="h-12 w-3/4 rounded-xl bg-zinc-800" />
              </div>
            </div>
          </motion.div>
        )}
        ...
        {uiState === "RESULT" && result && (
          <motion.div
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="w-full max-w-md flex flex-col gap-6 z-10 pb-20"
            role="alert"
            aria-live="assertive"
          >
            {result.urgency === "CRITICAL" && (
              ...
              >
                <Button 
                  size="lg" 
                  className="w-full h-20 text-2xl font-black bg-red-600 hover:bg-red-700 text-white shadow-[0_0_30px_rgba(220,38,38,0.5)] animate-pulse"
            ...
            <Card className="border-0 shadow-2xl bg-zinc-900 overflow-hidden">
              ...
              <CardContent className="pt-6 space-y-6">
                <div>
                  <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-2 flex items-center">
                    <ShieldAlert className="w-4 h-4 mr-2" /> Analysis
                  </h3>
                  <p className="text-lg font-medium leading-snug">{result.analysis}</p>
                </div>

                <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                  <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3 flex items-center">
                  ...
                  <ul className="space-y-3">
                    {result.immediate_steps.map((step, idx) => (
                      <li key={idx} className="flex gap-3 text-base">
                        <Badge variant="outline" className="h-6 w-6 rounded-full flex items-center justify-center p-0 flex-shrink-0 border-primary text-primary">
                          {idx + 1}
                        </Badge>
                ...
                <Alert className="bg-cyan-950/30 border-cyan-800/50">
                  <AlertTitle className="text-cyan-400 font-bold mb-2">Paramedic Handoff Brief</AlertTitle>
                  <AlertDescription className="text-cyan-100 text-sm leading-relaxed">
            ...
            <Button variant="outline" size="lg" onClick={reset} className="w-full border-zinc-700 text-zinc-300">
        ...
        {uiState === "ERROR" && (
          ...
          >
            ...
            <Button size="lg" className="w-full mt-4 bg-white text-black hover:bg-zinc-200" onClick={reset}>
              Try Again
            </Button>
          </motion.div>
        )}
